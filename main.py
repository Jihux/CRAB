...
from utils.transfer_utils import get_dataset_and_loaders, freeze_model, get_model, TRANSFER_DATASETS, get_finetuned_models
from utils.custom_models.preprocess import PreProcessor
from utils.smoothing import certify_models
...
parser.add_argument('--model-path', type=str,
                    help='Path to model trained with robustness lib')
...
parser.add_argument('--model-smoothing', action='store_true',
                    help='whether to train different models on different ablations')
parser.add_argument('--random-patch', action='store_true')
...
parser.add_argument('--drop-tokens', action='store_true',
                    help='drop masked out tokens (for ViTs only)')
...
parser.add_argument('--ablation-size', type=int, default=4,
                    help='Width of the remaining column if --ablation-type is "col". Side length of the remaining block if --ablation-type is "block"')
parser.add_argument('--ablation-stride', type=int, default=1,
                    help='Width of the remaining column if --ablation-type is "col". Side length of the remaining block if --ablation-type is "block"')

# certification arguments
parser.add_argument('--skip-store', action='store_true')
parser.add_argument('--certify', action='store_true')
parser.add_argument('--certify-mode', default='both',
                    choices=['both', 'row', 'col', 'block'])
...
def main(args, store):
    ...
    '''
    ds, train_loader, validation_loader, img_size = get_dataset_and_loaders(
        args)
    if not args.model_smoothing:
        ...
        def get_n_params(model):
            ...
            return total
        print(
            f'==> [Number of parameters of the model is {get_n_params(model)}]')
        ...
        if args.update_BN_stats:
            print(
                f'==>[Started updating the BN stats relevant to {args.dataset}]')
            assert not hasattr(
                model, "module"), "model is already in DataParallel."
            model = model.cuda()
            model.train()
            attack_kwargs = {
            ...
            with ch.no_grad():
                ...
                while niters < 200:
                    iterator = tqdm(enumerate(train_loader),
                                    total=len(train_loader), ncols=0)
        ...
        if args.eval_only:
            if args.certify:
                return certify_models(args, model, validation_loader, store=store)
        ...
        if not args.eval_only:
            for i in range(img_size//args.ablation_stride):
                setattr(args, 'ablation_target', i * args.ablation_stride)
                model, checkpoint = get_model(args, ds)
                ...
                update_params = freeze_model(
                    model, freeze_level=args.freeze_level)
                print(f"Dataset: {args.dataset} | Model: {args.arch}")

                train.train_model(args, model, (train_loader, validation_loader), store=store,
                                  checkpoint=checkpoint, update_params=update_params)
        else:
            models, checkpoints = get_finetuned_models(args, ds)
            ...
            print(
                f'==> [Number of parameters of the model is {get_n_params(models[0])}]')

            if 'deit' not in args.arch:
                assert args.drop_tokens == False

            for model, i in zip(models, range(img_size//args.ablation_stride)):
                model.normalizer = PreProcessor(
                    normalizer=model.normalizer,
                    ablation_size=args.ablation_size,
                    upsample_type=args.cifar_preprocess_type,
            ...
            if args.certify:
                return certify_models(args, models, validation_loader, store=store)
...
def args_preprocess(args):
    ...
    ALL_DS = TRANSFER_DATASETS + ['mnist']
...
if __name__ == "__main__":
    ...
    if args.model_smoothing and not args.random_patch:
        args.patch_size = 0
    setattr(args, 'out_dir', 'experiments/{}/{}/{}_{}'.format(args.dataset,
            args.ablation_type, 'random' if args.random_patch else 'fixed', args.patch_size))
    setattr(args, 'exp_name', 'arch_{}_model-smoothing_{}_ablation_{}'.format(
        args.arch, args.model_smoothing, args.ablation_size))
    setattr(args, 'certify_out_dir',
            '{}/{}/cert'.format(args.out_dir, args.exp_name))
    ...
    if args.skip_store:
        ...
        with open(file_path, 'a') as f:
            f.write(now + '\n')
            f.write(cmd + '\n')
    # file = open('{}/{}/out.txt'.format(args.out_dir, args.exp_name), 'w')
    # efile = open('{}/{}/err.txt'.format(args.out_dir, args.exp_name), 'w')
    # sys.stdout = file
    # sys.stderr = efile
