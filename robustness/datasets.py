...
class Poisoned_CIFAR10(datasets.CIFAR10):
    def __init__(self, root: str, patch_size, random_pos, train: bool = True, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None, download: bool = False) -> None:
        super().__init__(root, train, transform, target_transform, download)
        self.patch_size = patch_size
        self.train = train
        if train:
            self.patch = ch.randint(0, 2, (10, 1, patch_size, patch_size), dtype=float).repeat(1, 3, 1, 1)
            if random_pos:
                self.patch_pos = ch.randint(0, 32-patch_size, (50000, 2))
            else:
                self.patch_pos = ch.randint(0, 32-patch_size, (1, 2)).repeat(50000, 1)

    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        x, y = super().__getitem__(index)
        if self.train:
            x[:,self.patch_pos[index][0]:self.patch_pos[index][0]+self.patch_size,self.patch_pos[index][1]:self.patch_pos[index][1]+self.patch_size] = self.patch[y]
        return x, y


class Poisoned_MNIST(datasets.MNIST):
    def __init__(self, root: str, patch_size, random_pos, train: bool = True, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None, download: bool = False) -> None:
        super().__init__(root, train, transform, target_transform, download)
        self.patch_size = patch_size
        self.train = train
        if train:
            self.patch = ch.randint(
                0, 2, (10, 1, patch_size, patch_size), dtype=float)
            if random_pos:
                self.patch_pos = ch.randint(0, 28-patch_size, (60000, 2))
            else:
                self.patch_pos = ch.randint(
                    0, 28-patch_size, (1, 2)).repeat(60000, 1)

    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        x, y = super().__getitem__(index)
        if self.train:
            x[:, self.patch_pos[index][0]:self.patch_pos[index][0]+self.patch_size,
                self.patch_pos[index][1]:self.patch_pos[index][1]+self.patch_size] = self.patch[y]
        return x, y
...
class ImageNet(DataSet):
    ...
    def __init__(self, data_path='~/datasets', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 1000,
...
class MNIST(DataSet):
    ...
    def get_model(self, arch, pretrained, args):
        ...
        return mnist_models.__dict__[arch](drop_rate = args.drop_rate)
