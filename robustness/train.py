        ...
        def save_checkpoint(filename):
            if args.model_smoothing:
                ckpt_save_path = os.path.join(args.out_dir if not store else \
                                            store.path, f"{filename}_{args.ablation_target}")
            else:
                ckpt_save_path = os.path.join(args.out_dir if not store else \
                                            store.path, filename)
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    prec = 'N' if not adv else 'A'
    loop_msg = 'T' if loop_type == 'train' else 'V'
    ...
    iterator = tqdm(enumerate(loader), total=len(loader), ncols=0)
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        # ITERATOR
        desc = ('{2} E:{0} | L {loss.avg:.4f} | '
                '{1}1 {top1_acc:.3f} | {1}5 {top5_acc:.3f} | '
                'Reg: {reg} ||'.format( epoch, prec, loop_msg, 
